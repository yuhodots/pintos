...
/* Initializes the threading system by transforming the code
   ...
   After calling this function, be sure to initialize the page    // Page Allocator를 init 하라 !!
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  sema_init(&t->wait_sema, 0);                      /* 새로 생성한 thread(child)의 wait lock 초기화 */
  struct thread * parent = running_thread();        /* 현재 실행 중인 thread(parent) */
  list_push_back (&parent->child_list, &t->celem);  /* parent thread의 child list에 child thread의 elem 삽입 */
